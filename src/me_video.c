...
void me_readVertices(Polygon * polygon, const uint8_t *p, uint16_t zoom) {
	bbw = (*p++) * zoom / 64;
	bbh = (*p++) * zoom / 64;
	numPoints = *p++;
...
void me_init_video(Video * video, Resource *resParameter, System *stub){
	res = resParameter;
	sys = stub;
}
...
void fillPolygon(Video *video, uint16_t color, uint16_t zoom, const Point &pt) {
...
*/
void me_readAndDrawPolygonHierarchy(Video * video, uint16_t zoom, const Point &pgc) {
...
void me_drawString(Video * video, uint8_t color, uint16_t x, uint16_t y, uint16_t stringId) {
...
void me_drawChar(Video * video, uint8_t character, uint16_t x, uint16_t y, uint8_t color, uint8_t *buf) {
	if (x <= 39 && y <= 192) {
...
uint8_t *me_getPage(Video * video, uint8_t page) {
	...
	if (page <= 3) {
		...
		default:
			...
			warning("Video::megetPage() p != [0,1,2,3,0xFF,0xFE] == 0x%X", page);
			break;
		}
	}
	return p;
...
void me_changePagePtr1(Video * video, uint8_t pageID) {
...
void me_fillPage(Video * video, uint8_t pageId, uint8_t color) {
...
void me_opyPage(Video * video, uint8_t srcPageId, uint8_t dstPageId, int16_t vscroll) {
...
*/
void me_changePal(Video * video, uint8_t palNum) {
...
void me_updateDisplay(Video * video, uint8_t pageId) {
